...
| `copy-to-clipboard` | `.rd-js-copy` (delegated); `data-copy` / `data-copy-target` stay as payload API | default glyph (`content_copy`) is baked into `.copy-data-icon` CSS, so `data-icon` is **optional** in markup (`v0.6.17_1`); the script sets `data-icon="check"` on success and removes it on reset; `data-copy-busy` stays internal | script sets `aria-label="Copy"` on bare icon-only buttons (respects author-provided `aria-label`/`title`/text) so markup needs no `title`; live-region announcement still deferred to `CSS-231` | JS reads of `.copy-data-icon` as hook (class stays presentational) |
