...
| `copy-to-clipboard` | `.rd-js-copy` (delegated — any element, a whole block included; the copy icon is a carrier, not a requirement); payload API `data-copy-text` (a literal, checked first — `v3.2.0`) / `data-copy` / `data-copy-target` | default glyph (`content_copy`) is baked into `.copy-data-icon` CSS, so `data-icon` is **optional** in markup (`v0.6.17_1`); the script sets `data-icon="check"` **and** `.rd-is-copied` on success and removes both on reset (`v3.2.0`) — `data-icon` drives only the `.copy-data-icon` glyph swap, `.rd-is-copied` is the general state for every other carrier; `data-copy-busy` stays internal | script sets `aria-label="Copy"` on bare icon-only buttons (respects author-provided `aria-label`/`title`/text) so markup needs no `title`; live-region announcement still deferred to `CSS-231` | JS reads of `.copy-data-icon` as hook (class stays presentational) |
...
| `carousel` | `.rd-js-carousel` on the root (scope), `.rd-js-carousel-track` (its **element children are the slides** — the script never reads the `.carousel-slide` styling class), `.rd-js-carousel-prev` / `-next` on the arrow buttons, `.rd-js-carousel-dots` on the **optional** container the script fills (absent → no dots built) | `.rd-is-active` on the current slide and its generated dot | `role="group"` + `aria-roledescription="carousel"` on the root; arrows/dots are real `<button>`s with labels; `aria-hidden` on inactive slides; `aria-current` on the active dot; `←`/`→` keys | (harvest — no legacy hooks to remove; see note) |
