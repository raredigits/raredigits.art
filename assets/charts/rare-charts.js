...
var RareCharts = (() => {
  ...
  function renderAxisTitles(g, W, y1Title, y2Title, theme) {
    ...
    g.selectAll(".rc-axis-title").data(titles, (d) => d.axis).join("text").attr("class", (d) => `rc-axis-title rc-axis-title-${d.axis}`).attr("x", (d) => d.axis === "y1" ? W + 8 : -8).attr("y", -28).attr("fill", theme.text).style("font-family", theme.font).style("font-size", theme.fontSize).text((d) => d.text);
  }
  ...
  var TimeSeries = class extends Chart {
    ...
    appendPoint(point6) {
      this._data.push(point6);
      this._data.sort((a4, b) => a4.date - b.date);
      this.render();
      return this;
    }
    setView(extent2) {
    ...
    _initSVG() {
      ...
      this.clipRect = this.svg.append("defs").append("clipPath").attr("id", clipId).append("rect");
      this.clipId = clipId;
      this.gGrid = this.g.append("g").attr("class", "rc-grid");
    ...
    render() {
      ...
      if (areaOpt !== false) {
        ...
        if (solidOpacity !== null) {
          this.gPaths.selectAll(".rc-ts-area").data([visible]).join("path").attr("class", "rc-ts-area").attr("d", area).attr("fill", areaColor).attr("fill-opacity", solidOpacity);
        } else {
          ...
          this.gPaths.selectAll(".rc-ts-area").data([visible]).join("path").attr("class", "rc-ts-area").attr("d", area).attr("fill", "url(#rc-ts-grad)").attr("fill-opacity", 1);
        }
      } else {
    ...
    _bindZoomPan() {
      this._zoom = zoom_default2().scaleExtent([1, 500]).on("zoom", (event) => {
        if (!this._data.length) return;
        const fullExtent = extent(this._data, (d) => d.date);
        const xFull = time().domain(fullExtent).range([0, this.width]);
        const newX = event.transform.rescaleX(xFull);
        const extent2 = [
          max([newX.invert(0), fullExtent[0]]),
    ...
    _bindHover() {
      this.overlay.on("mousemove", (event) => {
        ...
        const [mx] = pointer_default(event);
        const W = this.width, H = this.height;
        const date2 = this.xScale.invert(mx);
        const bisect = bisector((d2) => d2.date).left;
        const idx = bisect(this._visible, date2, 1);
        const d0 = this._visible[idx - 1];
        const d1 = this._visible[idx];
        const d = d1 && date2 - d0.date > d1.date - date2 ? d1 : d0 || d1;
        if (!d) return;
  ...
  var DualAxes = class extends Chart {
    ...
    _renderBars({ bars, allValues, x: x4, y1: y12, y2: y22, barOpacity, barWidthRatio, barGrouping, animate, duration, ease }) {
      ...
      barsSel.call((sel) => {
        const apply = (sel2) => sel2.attr("x", (d) => x4(d.date) - barW / 2 + xOffset(d.series)).attr("width", barW).attr("y", (d) => scaleFor(d)(Math.max(0, d.value))).attr("height", (d) => Math.abs(scaleFor(d)(d.value) - scaleFor(d)(0)));
  ...
  // assets/charts/src/charts/Gauge.js
  var Gauge = class extends Chart {
    constructor(selector, options = {}) {
      super(selector, {
        height: 220,
        margin: { top: 8, right: 8, bottom: 8, left: 8 },
    ...
    render() {
      ...
      const o = this.options;
      const t = this.theme;
      const startAngle = o.startAngle ?? -Math.PI * 0.75;
      const endAngle = o.endAngle ?? Math.PI * 0.75;
      ...
      const fraction = this._max - this._min > 0 ? (clamp(this._value) - this._min) / (this._max - this._min) : 0;
      const fillEnd = startAngle + fraction * (endAngle - startAngle);
      const arcBase = () => arc_default().innerRadius(innerR).outerRadius(outerR).cornerRadius(cornerR);
      const trackArc = arcBase().startAngle(startAngle).endAngle(endAngle);
      const fillArc = arcBase().startAngle(startAngle);
      this.gArc.selectAll(".rc-gauge-track").data([null]).join("path").attr("class", "rc-gauge-track").attr("fill", trackColor).attr("d", trackArc());
      const fillPath = this.gArc.selectAll(".rc-gauge-fill").data([null]).join("path").attr("class", "rc-gauge-fill").attr("fill", fillColor);
      if (animate) {
        fillPath.attr("d", fillArc.endAngle(startAngle)()).transition().duration(duration).ease(ease).attrTween("d", () => {
          const interp = value_default(startAngle, fillEnd);
          return (t4) => fillArc.endAngle(interp(t4))();
        }).on("end", () => {
          this._didAnimateIn = true;
        });
      } else {
      ...
      this.gArc.selectAll("path").on("mousemove", (event) => {
        const [mx, my] = pointer_default(event, this.container);
  ...
  var Graph = class extends Chart {
    ...
    _initSVG() {
      this.container.style.height = this.options.height + "px";
      this.svg = select_default2(this.container).append("svg").attr("width", "100%").attr("height", "100%").style("cursor", "grab").style("overflow", "hidden");
    ...
    render() {
      ...
      const nodeSel = this.gNodes.selectAll(".rc-graph-node").data(this._nodes, (d) => d.id).join(
        (enter) => {
          ...
          g.each(function() {
            const grp = select_default2(this);
            ...
            const padX2 = 3, padY2 = 1;
            bg.attr("x", bbox.x - padX2).attr("y", bbox.y - padY2).attr("width", bbox.width + padX2 * 2).attr("height", bbox.height + padY2 * 2);
          });
          return g;
        },
        (update) => update,
      ...
      ).force("charge", manyBody_default().strength((d) => charge * Math.max(1, (d.size ?? 1) * 0.75)).distanceMax(W * 0.7)).force("center", center_default(W / 2, H / 2).strength(0.03)).force("collide", collide_default().radius((d) => nodeR(d) + 20).strength(0.9)).force("clusterX", x_default2((d) => clusterX[d.group ?? "default"] ?? W / 2).strength(0.07)).force("clusterY", y_default2((d) => clusterY[d.group ?? "default"] ?? H / 2).strength(0.07)).on("tick", () => {
  ...
  // assets/charts/src/adapters/index.js
  async function fromJson(url, mapping) {
    const raw = await json_default(url);
    return applyMapping(Array.isArray(raw) ? raw : raw.data ?? Object.values(raw), mapping);
  }
  async function fromCsv(url, mapping) {
    const raw = await csv2(url);
    return applyMapping(raw, mapping);
  }
  async function fromApi(url, mapping, options = {}) {
  ...
  function fromArray(data, mapping) {
  ...
  // assets/charts/src/index.js
  function injectCssOnce(id2, cssText) {
  ...
  function generateMockPrices(days = 365, startPrice = 150) {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      data.push({
        ...
        volume: Math.floor(5e7 + Math.random() * 1e8)
      });
    }
    return data;
  }
